"""
A Python Shell for Immunity Debugger, with tab completion

Kiran Bandla <kbandla@in2void.com>,  2009
Multiple fixes to make the project work with new python objects
...
__VERSION__  = '1.5.1'
NAME        = 'shell'
DESC        = 'Interactive python shell for Immunity Debugger'
COPYRIGHT   = 'No Copyright. This script is public domain'
LICENSE     = 'WTFPL'
...
class Console(Frame):
    ...
    def precontext(self):
        '''
        @kbandla
        TODO: Fix precontext to not populate function arguments
        '''
    ...
    def cb_complete(self, event):
        ...
        #@kbandla: Fix bug when command has trailing tabs
        if not context:
            return "break"
...
def members(Object):
    '''
    @kbandla
    dont use __members__, __methods__, __bases__. 
    using inspect module to achieve the same results
    '''
    resultD = dict(inspect.getmembers(Object))
    builtinsL = dict(inspect.getmembers(Object, inspect.isbuiltin)).keys()
    """
    if type(Object) is types.ClassType:
        scanclass(Object, resultD)
        result["__name__"] = 1
        result["__bases__"] = 1
    if type(Object) is types.InstanceType:
        scanclass(Object.__class_$_, resultD)
        result["__class__"] = 1
    """
    result = set(resultD.keys()).difference(set(builtinsL))
    return list(result)
...
def immLog(msg, address='BADFOOD', highlight=False, gray=False, focus=0):
    global imm
    imm.log(msg, address, highlight, gray, focus)
...
def main(aaa):
    global imm
    sys.argv = [os.getcwd()]
    c = Console(dict={'imm':imm})
    c.dict["console"] = c
    c.pack(fill=BOTH, expand=1)
    c.master.title("Immunity Console v%s" % VERSION)
    mainloop()
    return 'Done'
